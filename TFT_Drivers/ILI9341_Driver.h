...
class ILI9341_Driver : public Driver, public CommandExecutor {
public:
    explicit ILI9341_Driver(const Configuration& cfg);
    ~ILI9341_Driver() override = default;
    ...
    static bool validateConfig(const Configuration& config) {
        ...
        // Validate dimensions
        if (config.geometry.width != 240 || config.geometry.height != 320) {
            return false;
        }
