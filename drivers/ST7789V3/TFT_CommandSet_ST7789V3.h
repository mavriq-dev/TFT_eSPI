#ifndef _TFT_COMMANDSET_ST7789V3_H_
#define _TFT_COMMANDSET_ST7789V3_H_

#include "../../TFT_Command.h"

namespace TFT_Runtime {

class TFT_CommandSet_ST7789V3 : public CommandSequenceFactory {
public:
    CommandSequence createInitSequence() override {
        CommandSequence seq("ST7789V3 Init");
        ...
        seq.addDataList({0xD0, 0x08, 0x0E, 0x09, 0x09, 0x05, 0x31, 0x33,
                        0x48, 0x17, 0x14, 0x15, 0x31, 0x34});
        
        // Negative Voltage Gamma Control
        seq.addCommand(0xE1, "Negative Voltage Gamma Control");
        seq.addDataList({0xD0, 0x08, 0x0E, 0x09, 0x09, 0x15, 0x31, 0x33,
                        0x48, 0x17, 0x14, 0x15, 0x31, 0x34});
    ...
    CommandSequence createSleepSequence() override {
        CommandSequence seq("ST7789V3 Sleep");
    ...
    CommandSequence createWakeSequence() override {
        CommandSequence seq("ST7789V3 Wake");
        seq.addCommand(0x11, "Sleep Out");
        seq.addDelay(120);
        seq.addCommand(0x29, "Display On");
        seq.addDelay(20);
    ...
    CommandSequence createDisplayOnSequence() override {
        CommandSequence seq("ST7789V3 Display On");
        seq.addCommand(0x29, "Display On");
        seq.addDelay(20);
        seq.addEnd();
        return seq;
    ...
    CommandSequence createDisplayOffSequence() override {
        CommandSequence seq("ST7789V3 Display Off");
        seq.addCommand(0x28, "Display Off");
        seq.addDelay(20);
        seq.addEnd();
        return seq;
    ...
    CommandSequence createInvertOnSequence() override {
        CommandSequence seq("ST7789V3 Invert On");
        seq.addCommand(0x21, "Display Inversion ON");
        seq.addEnd();
        return seq;
    }
    ...
    CommandSequence createInvertOffSequence() override {
        CommandSequence seq("ST7789V3 Invert Off");
    ...
    CommandSequence createSetRotationSequence(uint8_t rotation) {
        CommandSequence seq("ST7789V3 Set Rotation");
        uint8_t madctl = 0;
    ...
    CommandSequence createSetWindowSequence(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1) {
        CommandSequence seq("ST7789V3 Set Window");
    ...
    CommandSequence createSetGammaSequence(bool positive, const std::vector<uint8_t>& gammaValues) {
        CommandSequence seq("ST7789V3 Set Gamma");
    ...
    CommandSequence createSetVCOMSequence(uint8_t vcom) {
        CommandSequence seq("ST7789V3 Set VCOM");
        seq.addCommand(0xBB, "VCOM Setting");
        seq.addData(vcom);
        seq.addEnd();
        return seq;
    ...
    CommandSequence createSetPowerControlSequence(const std::vector<uint8_t>& powerParams) {
        CommandSequence seq("ST7789V3 Set Power Control");
...
#endif // _TFT_COMMANDSET_ST7789V3_H_
